...
namespace Fighter
{
    ...
    public class AfterbunnerEffect : MonoBehaviour
    {
        ...
        private FighterController m_Jet; // The jet that the particle effect is attached to
        ...
        private FighterController FindFighterParent()
        {
            ...
            while (t != null)
            {
                var aero = t.GetComponent<FighterController>();
                if (aero == null)
                {
                    // try next parent
                    t = t.parent;
