...
{
    [ExecuteInEditMode]
    public class FighterCamera : PivotBasedCameraRig
    {
        [SerializeField] private float m_MoveSpeed = 3; //ターゲットの位置に追いつくためにリグが移動する速度
        [SerializeField] private float m_TurnSpeed = 1; // ターゲットの回転に追いつくためにリグがどれだけ速く回転するか
        [SerializeField] private float m_RollSpeed = 0.2f;// ターゲットのロールに一致するようにリグがロールする速度（Z軸の周り）。
        [SerializeField] private bool m_FollowVelocity = false;// リグがターゲットの速度の方向に回転するかどうか。
        [SerializeField] private bool m_FollowTilt = true; // リグがターゲットに対して傾斜するかどうか(X軸の周り)。
        [SerializeField] private float m_SpinTurnLimit = 90;// ターゲットの回転に続いてカメラが停止するしきい値。(戦闘機がぶつかってスピンするときに)
